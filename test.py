import pygame
import sys
from plateau import (Plateau, Joueur, dessiner_plateau, dessiner_infos_partie,
                      LARGEUR_FENETRE, HAUTEUR_FENETRE, NOIR)
...
def lancer_plateau():
    """Lance la fenêtre du plateau de jeu, puis revient au menu."""
    # Redimensionner la fenêtre pour le plateau
    ecran_jeu = pygame.display.set_mode((LARGEUR_FENETRE, HAUTEUR_FENETRE))
    pygame.display.set_caption("Legends Strikes - Plateau")

    plateau = Plateau()
    joueur_atk = Joueur("Attaquant", "attaquant")
    joueur_def = Joueur("Défenseur", "defenseur")
    tour = 1
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                if event.key == pygame.K_ESCAPE:
                    en_jeu = False  # retour au menu

        ecran_jeu.fill(NOIR)
        dessiner_plateau(ecran_jeu, plateau)
        dessiner_infos_partie(ecran_jeu, joueur_atk, joueur_def, tour)

        # Afficher les coordonnées de la case survolée par le curseur
        pos_souris_jeu = pygame.mouse.get_pos()
        case = plateau.pixel_vers_case(*pos_souris_jeu)
        police_coord = pygame.font.Font(None, 24)
        if case:
            col, ligne = case
            txt = police_coord.render(f"Case: ({col}, {ligne})", True, (255, 255, 255))
        else:
            txt = police_coord.render("Hors grille", True, (150, 150, 150))
        ecran_jeu.blit(txt, (LARGEUR_FENETRE - 160, HAUTEUR_FENETRE - 30))

        pygame.display.flip()
        clock_jeu.tick(60)

    # Revenir à la taille du menu
